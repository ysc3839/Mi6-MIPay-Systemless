...
wget $MIUI_CN_STABLE_ZIP || exit 1
unzip $(basename $MIUI_CN_STABLE_ZIP) system.transfer.list system.new.dat || exit 1
rm $(basename $MIUI_CN_STABLE_ZIP)
...
zip -r -x *.git* build.sh *.txt *.py *.zip *.yml -y -9 Mi6-MIPay-Systemless-MIUI-Stable-$version.zip . || exit 1
